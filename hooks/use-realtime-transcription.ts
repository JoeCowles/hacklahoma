...
async function fetchRealtimeToken(): Promise<string> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      body.error || "Failed to create ElevenLabs realtime token"
    );
  }
...
export function useRealtimeTranscription(): UseRealtimeTranscriptionReturn {
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      const socket = new WebSocket(
        `${ELEVENLABS_REALTIME_URL}?${params.toString()}`
      );
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (data.message_type === "partial_transcript" && data.text) {
            ...
            setTranscripts((prev) => {
              ...
              if (last?.type === "partial") {
                return [
                  ...prev.slice(0, -1),
                  { ...last, text: transcriptText },
                ];
              }
              return [
                ...prev,
                { time: formatTime(), text: transcriptText, type: "partial" },
              ];
            });
            return;
          }
          ...
          if (
            ...
            const lectureId =
              "lecture_" + new Date().toISOString().split("T")[0];
            ...
            fetch("http://localhost:8000/concepts/extract", {
              ...
              .then((res) => res.json())
              .then((d) => console.log("Extracted Concepts:", d))
              .catch((err) =>
            ...
            setTranscripts((prev) => {
              ...
              if (last?.type === "partial") {
                return [
                  ...prev.slice(0, -1),
                  { ...last, text: transcriptText, type: "committed" },
                ];
              }
              return prev;
            });
  ...
  }, [cleanup, isRecording, transcripts]);
